 ...
 */
    private Card selectCardFromHand(Player currentPlayer) {
        Card card = null;
        Random random = new Random();
        if(!currentPlayer.getName().equals("PC")) {
            ...
            if(humanPlayerCard.size() > 0) {
                ...
                System.out.println();
                System.out.println("Do you want to exit the game, Enter true or false ");

                boolean quit =  new Scanner(System.in).nextBoolean();
                if(quit){
                    quitGame();
                }
    ...
    public void quitGame() {
        CasinoDriver driver = new CasinoDriver();
    }
